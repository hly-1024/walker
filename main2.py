...
def select_best_objective_index(F: np.ndarray, utility_tol: float = 1e-6) -> int:
    """Pick max utility, then fewer active dual-mode satellite roles."""
...
class Config:
    ...
    # Communication model and cross-layer access topology controls.
    ...
    # Task generation and utility model. TASK_GEN_RATE is the total task data
    # released per simulated second. R_SENSOR is the per-satellite sensing
    # ingestion cap. Virtual observation nodes are generated implicitly from
    # task_coverage_by_region, task_region_weights and satellite buffers.
    ...
    # NSGA-III sparse activation optimization parameters.
    GA_POP: int = 120
    GA_GEN: int = 60
    GA_CR: float = 0.9
    GA_PARTITIONS: int = 4
...
class DataLoader:
    ...
    def _compute_ground_geometry(self) -> None:
        ...
        # implicit virtual observation node O(t, i, r).
...
class SimulationEngine:
    ...
    def split_genes(self, genes: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        The physical coupling constraint comm_on |= sense_on is retained:
        a dual-mode satellite that accepts a sensing role must also activate
        its communication forwarding role so generated data can be buffered,
        forwarded through ISL links, and downlinked to the Beijing ground station.
        """
        N = self.dl.N
        ...
        sense_on = g[:N].copy()
        comm_on = g[N:].copy()
        # Sensing role depends on communication role for cache/forwarding/downlink.
        comm_on |= sense_on
    ...
    def _isl_transfer(
        ...
        """Run online ISL forwarding over the current cross-layer topology.

        use_topk=False builds a Full Graph candidate set among active
        communication forwarding nodes. use_topk=True uses the precomputed
        cKDTree Top-K sparse candidate graph and then adaptively clips each
        sender's local K according to buffer pressure, future GS visibility and
        task priority. matcher_mode selects Greedy or Hungarian online matching;
        this is separate from the outer NSGA-III sparse activation optimizer.
        """
        cfg = self.cfg
        dl = self.dl
    ...
    def evaluate(
        ...
        """Evaluate one sparse activation scheme over all time steps.

        Per time step, the simulator updates communication-role energy,
        applies eclipse/solar charging, base drain, low-power protection and
        melt-down buffer drops; then it implicitly generates virtual observation
        nodes from task-region coverage and sensing-role availability, stores
        data in q_region/q, runs ISL transfer on a Full Graph or Top-K sparse
        candidate graph with Greedy/Hungarian matching, and finally downlinks
        visible buffered data to the Beijing ground station.
        """
        cfg = self.cfg
        dl = self.dl
        ...
        # q_region[r, i] is the implicit buffer_state of virtual observation
        # nodes generated for task region r and currently stored on satellite i.
        ...
        # Remaining task volume is split by task_region_weights; together with
        # task_coverage_by_region it controls implicit O(t, i, r) generation.
        ...
        for t in range(dl.T):
            ...
            if remaining > 0.0:
                ...
                for region_i, region_weight in enumerate(dl.task_region_weights):
                    ...
                    used = alloc > 1e-9
                    if np.any(used):
                        # Implicit virtual observation nodes O(t, i, r) are
                        # materialized only as buffered MB in q_region.
        ...
        # delivery_ratio and packet_loss_rate deliberately use different
        # denominators: delivered/TASK_DATA_MB vs dropped/generated.
...
class NSGA3Solver:
    ...
    def optimize(self, eval_fn, n_sat: int) -> Dict:
        ...
        refs = self._ref_dirs(int(F.shape[1]), c.GA_PARTITIONS)
        history: List[Dict] = []
        ...
        for g in range(gen):
            ...
            # Active satellite count is the communication-role count because
            # comm_on |= sense_on makes every sensing node a forwarding node.
...
class ConstellationProblem:
    ...
    The decision vector has length 2N: genes[:N] are sensing roles and
    genes[N:] are communication forwarding roles. The objective vector is
    [-utility, n_sense, n_comm], so NSGA-III maximizes task delivery utility
    while minimizing sensing activation scale and communication activation
    scale. Online Greedy/Hungarian scheduling remains inside
    SimulationEngine.evaluate() and is not replaced by NSGA-III.
    """
...
def build_method_configs(dl: DataLoader, best_gene: np.ndarray) -> List[Dict]:
    ...
    # M1-M8 keep the same method_id contract used by tables and figures.
    # Names describe activation optimizer, candidate graph, and online matcher.
    return [
        {
            "method_id": "M1",
...
def run_method_comparison_experiment(
    ...
    for item in methods:
        ...
        summary["scoring"] = "Cross-layer score with Full/Top-K candidate graph and Greedy/Hungarian online scheduling"
...
def save_method_comparison_outputs(out_dir: Path, comparison: Dict) -> None:
    ...
    for m in methods:
        table1_new.append(
            {
                "方法": m.get("label", ""),
                "感知激活数": m.get("sense_active_count", 0),
                "通信转发激活数": m.get("comm_active_count", 0),
    ...
    pd.DataFrame(table1_new).to_csv(out_dir / "table1_method_performance.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(table2_new).to_csv(out_dir / "table2_energy_protection.csv", index=False, encoding="utf-8-sig")
...
def run_scalability_experiment(
    ...
    return {
        ...
        "repeats": int(repeats),
        "delivery_ratio_definition": "delivered_mb/TASK_DATA_MB within sampled scale benchmark",
        "packet_loss_rate_definition": "dropped_mb/generated_mb; not complementary to delivery_ratio",
        "online_time_definition": "online ISL matching plus GS downlink selection only",
        "total_runtime_definition": "online scheduling time plus scaled NSGA-III sparse activation runtime for NSGA-III methods",
...
class Visualizer:
    ...
    def fig1_topology(self, dl: DataLoader, best_genes: np.ndarray) -> None:
        ...
        ax.scatter(px[::step], py[::step], c="#6BAED6", s=10, alpha=0.52, zorder=3, label=f"Candidate forwarding node ({dl.N})")
        if np.any(cover):
        ...
        if np.any(comm_only):
            ...
            ax.scatter(
                ...
                linewidths=0.3,
                zorder=6,
                label=f"Active communication role ({len(comm_idx)})",
            )
        if np.any(sense_on):
            ...
            ax.scatter(
                ...
                linewidths=0.8,
                zorder=8,
                label=f"Active sensing role ({len(sense_idx)})",
            )
        ...
        for sat in active_sense[:30]:
            ...
            if drawn >= 24:
                break
        ax.set_title("(b) Sparse cross-layer access topology", fontsize=12, fontweight="bold")
        ax.legend(loc="upper left", fontsize=8, framealpha=0.9)
        ax.set_xlim(-10500, 10500)
        ax.set_ylim(-9500, 9500)

        fig.suptitle("Fig 1. Cross-layer access topology snapshot", fontsize=14, fontweight="bold", y=1.01)
    ...
    def fig4_pareto_convergence(self, F: np.ndarray, history: List[Dict]) -> None:
        ...
        comm_role = np.asarray([h.get("comm_count", 0) for h in history], dtype=float)
        total = np.asarray(
            [h.get("total_active", h.get("comm_count", 0)) for h in history],
            dtype=float,
        )
        near_best_min = np.asarray(
            [
                h.get(
                    ...
                    h.get("total_active", h.get("comm_count", 0)),
                )
                for h in history
            ],
            dtype=float,
        ...
        ax.plot(
            ...
            markerfacecolor="white",
            markeredgewidth=1.2,
            label="Active communication role",
        )
        ax.plot(
            gens,
            total,
        ...
        ax.set_title("Sparse activation convergence", fontsize=11.5, fontweight="bold")
        ...
        if len(gens) > 0:
            ...
            ax.text(
                0.98,
                0.94,
                f"Final: sensing={int(sense[-1])}, comm={int(comm_role[-1])}, active={int(total[-1])}",
        ...
        y_min = min(float(np.min(sense)), float(np.min(comm_role)), float(np.min(total)))
        y_max = max(float(np.max(sense)), float(np.max(comm_role)), float(np.max(total)), float(np.max(near_best_min)))
        ...
        fig.suptitle("Fig. 4  NSGA-III Sparse Activation Convergence", fontsize=13.5, fontweight="bold", y=0.98)
    ...
    def fig4_activation_utility_3d(self, X: np.ndarray, F: np.ndarray, dl: DataLoader, best_gene: np.ndarray) -> None:
        ...
        comm_count = np.sum(X[:, dl.N :] > 0.5, axis=1).astype(float)
        total_active = sense_count + comm_count
        ...
        ax.set_xlabel("Communication forwarding active count", fontsize=11)
        ax.set_ylabel("Sensing active count", fontsize=11)
        ax.set_title("Fig. 4-S  Sparse Activation-Utility Distribution", fontsize=12.5, fontweight="bold", pad=12)
    ...
    def fig5_complexity_overall(self, timing: Dict) -> None:
        ...
        line3, = ax.plot(
            ...
            mfc="none",
            mew=1.9,
            label="NSGA-III + Top-K Graph + Hungarian",
            zorder=7,
        )
        ...
        ax.text(
            0.985,
            0.035,
            "Total runtime includes NSGA-III sparse activation + online scheduling",
            transform=ax.transAxes,
            ha="right",
            va="bottom",
            fontsize=8.8,
    ...
    def fig6_baseline_comparison(self, compare: Dict) -> None:
        ...
        metrics = [
            ...
            ("Simulation runtime", runtime / max(float(runtime[0]), 1e-9), "Lower is better"),
        ]
    ...
    def fig_topology(self, dl: DataLoader, sense_on: np.ndarray, comm_on: np.ndarray) -> None:
        ...
        ax.scatter(
            ...
            c="#de2d26",
            alpha=0.9,
            label="sensing role + communication role",
        )
        drawn = 0
        ...
        for i in comm_idx[: min(120, len(comm_idx))]:
            ...
            if drawn >= 80:
                break
        ax.set_title("Cross-layer access topology snapshot")
    ...
    def fig_activation(self, sat_names: np.ndarray, sense_on: np.ndarray, comm_on: np.ndarray) -> None:
        ...
        axes[0].set_yticks([1])
        axes[0].set_yticklabels(["comm role"])
        axes[0].set_title("Communication role activation")
        axes[1].scatter(x[sense_on], np.ones(int(sense_on.sum())), s=10, c="#de2d26")
        axes[1].set_ylim(0.5, 1.5)
        axes[1].set_yticks([1])
        axes[1].set_yticklabels(["sense role"])
        axes[1].set_xlabel("Satellite index")
        axes[1].set_title("Sensing role activation")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cross-layer topology optimization for dual-mode constellation sensing/communication simulation"
    )
